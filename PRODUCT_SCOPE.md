...
- CLI + MCP.
- Spatial board visor (`jarvis board`) — **read-only** projection of `ProjectState` (cards + architecture slots; declared geometry as text, 2D glyphs, and CSS 3D solids when envelope exists; click-inspect; declared box-local pose as **card text** today). Not a v1 mutation surface; CLI/MCP remain the engineering I/O. **Geometry queue:** [3D mapping path](.jes/artifacts/engineer_lock_geometry_3d_mapping_path.md) — Scene3D-from-pose investigation first; then optional wheelbase/N-motor sketch, cited motor height, sourced plate L×W; `"cabe"` last. Drag of cards is layout, not pose. One `ComponentSpec` = one card/solid; `motor_count` is a property, not N nodes, until a later named ★.
