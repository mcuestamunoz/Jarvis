...
| D3 | módulo `design_utils.py` no existe — helpers de lectura no pueden vivir en `orchestrator.py` | Fase 3 (segundo helper de lectura) |
| D4 | **Bypass de mirrors via signal de params**: `param_definition_session` puede escribir `motor_power_w` o `battery_capacity_wh` directamente en `current_parameters` sin pasar por `_set_motor_component` / `_set_battery_component` — el `ComponentSpec` no se crea. El mismo riesgo existe ahora para `motor_count` + `per_motor_max_thrust_n` via `MISSING_PROPULSION_PARAMETERS`. Solución: gatekeeper en `set_param()` que rechace escritura directa de `COMPONENT_MIRRORED_PARAMS`, o filtrado en la capa de param wizard antes de escribir. | Fase 7 (gatekeeper de escritura) |
...
| D6 | **Dualidad física de `propellers`**: el componente `components["propellers"]` es declarativo (describe físicamente las hélices) pero el `calculation_engine` usa `propeller_diameter_in` + `propeller_rpm` como parámetros sueltos. No hay bridge entre el componente y los parámetros físicos. Consecuencia: el usuario puede declarar "hélices 10x4.5" y el engine seguir sin datos de empuje por hélice. Decisión futura: ¿`propellers` se convierte en fuente de verdad física? Si sí, añadir `_set_propeller_component()` bridge que extraiga `propeller_diameter_in` del spec y lo escriba en `current_parameters` (análogo a `_set_motor_component` → `motor_power_w`). | Fase 7 (bridge físico propellers) |
