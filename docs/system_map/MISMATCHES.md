...
Code currently (d224dc1):   composite energy block can still be in_progress because motor_power_w
                            (or catalog motor bind) is stale after re-declaring motors at
                            IDLE, while battery component + battery_capacity_wh remain set —
                            but in-progress labels are now computed dynamically from active
                            missing sub-parts instead of static marketing labels.
...
Resolution:                 CLOSED (post-checkpoint micro-fix `d224dc1`): `_block_label_for`
                            + composite in-progress label helper align architecture hints with
                            the actual active gap (e.g., motor_power_w vs battery).
```
...
| **H5** | Sim → Continuity, PASS + risky (addresses C-081) | ✅ **IMPLEMENTED (Claim Hygiene IC, 2026-09-04)** | Resolved the data-contract question this row left open: `build_project_continuity` gained `margin_claim_weak(sim)` — reads `quality`/`warnings` directly (no new threshold; reuses `simulator._resolve_quality`'s `"risky"` band and the `low_margin`/`high_actuator_load`/`low_force_to_weight_ratio` warning codes already computed) — gating one new **situation**-branch sentence (not the next-step branch this row originally described) before the generic "Diseño validado" fallback. A sibling gate, `_frame_class_gap_live(readiness)` (Structure Foundations IC, same date), covers the analogous case for a live frame-class gap. No structured "risk thread" field was introduced — the simpler of the two options this row named was sufficient. See `.jes/artifacts/investigation_report_claim_hygiene_assembly_ready.md`. |

**H1–H5 all closed (FN-024/FN-025/FN-026; H5 via Claim Hygiene IC 2026-09-04). No open H-row remains from this table** — the residual "PASS + any other live gap type" pattern (only margin/quality and frame-class were proven and fixed) is tracked as a named future thread in the claim-hygiene report, not a queued FN here.
