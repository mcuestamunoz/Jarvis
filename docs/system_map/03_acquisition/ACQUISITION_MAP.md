...
| `core/catalog_bind.py` | `bind_motor_from_catalog` / `bind_propeller_from_catalog` / `bind_battery_from_catalog` — shared bind primitive; `invalidate_diverged_catalog_refs` (G5 divergence clears `catalog_ref`; G24D sets motor `.name` to honest non-SKU label) |
