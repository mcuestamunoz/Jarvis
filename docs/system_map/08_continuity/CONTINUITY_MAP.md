...
**Inbound:** C-071 (from Simulation), C-060 (current_parameters, indirectly via requirements), **C-107** (authorities → `build_engineering_readiness`). **Outbound:** C-021/C-035 (to Runtime's `_handle_project_status`), C-036 (to Acquisition, shared `_next_pending_block` read), **C-108** (partial — catalog-gap ranking only, when `readiness=` supplied), **C-109/C-110** (startup context + CLI render).

## Key modules

| Path | Role |
|---|---|
| `core/engineering_readiness.py` | **ERF-1** — Gap Registry + eight-subsystem assembly-ready rollup; pure aggregator over closure/arch/sim (C-107). Not a write authority; does not replace BOM/sim/classifier logic. |
...
- `build_engineering_readiness(project_state) -> EngineeringReadinessResult` (`engineering_readiness.py`, ERF-1) — six gap types, eight subsystem lines (`requirements`…`bom` only — no electronics/comms/integration), derived on read. Consumed by orchestrator startup context (C-109) and optionally by Continuity (C-108).
