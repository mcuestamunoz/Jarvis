...
| `core/pose_envelope_screening.py` | AABB screening “cabe” for posed child vs origin — used by fit attestation + fit-relations checklist (suggest path only) |
| `core/mount_standard_assist.py` | IDLE mount checklist (suggest phrases) — closed B1 |
| `core/silhouette_product_b_assist.py` | IDLE `parece un dron` — **declared checklist** (B / B\*), never visual recognition |
| `core/fit_relations_assist.py` | IDLE `relaciones` / `fit` — 6 locked fit relations; status ladder; suggest-only; estimated plate blocks attest |
| `core/craft_montage_stack_assist.py` | Path F stack suggest on plate (reused by silhouette / fit-relations plate pick) |
| `core/layout_pack_assist.py` | IDLE cited-layout-pack suggest (reused by silhouette / fit-relations plate pick) |
| `core/estimated_temporary_plate_assist.py` | IDLE `declara ... estimad[ao]/provisional` — writes a Path-F plate L×W×H box at `confidence=0.3`; blocks fit attestation |
| `core/estimated_temporary_esc_assist.py` | IDLE `declara el esc estimado/temporal/provisional <N> mm` — single-axis ESC height completion, same `estimated_temporary` convention; blocks fit attestation |
