...
| `domains/aerial.py`, `domains/ground.py` | Data: keyword tables + property extractors per domain. **Structure B:** `extract_frame_properties` (+ configuration/wheelbase), `extract_all_frame_part_properties` / `extract_frame_part_properties` (G-N1 clause-scoped parts; arm-clause `thickness_mm` only); `frame_plate_key` / `is_frame_plate_key` / `FRAME_PLATE_MAX_SIBLINGS=8`. **Geometry FC B1:** `FLIGHT_CONTROLLER_DIMENSIONS` identity-linked box for `pixhawk_4` (no catalog). |
