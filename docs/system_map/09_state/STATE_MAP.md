...
| `workspace/spatial_board.py` | `project_spatial_nodes` — `ProjectState` → visor DTOs (`component` / `part` / B3 `slot`). Read-only; not C-094 markdown; not a writer. Slots = missing `BLOCK_TO_COMPONENTS` keys of **declared** `system_blocks` only. |
