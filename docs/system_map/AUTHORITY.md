...
| Which gaps exist + assembly-ready rollup (8 subsystem lines)? | `engineering_readiness.build_engineering_readiness` (ERF-1) — composes closure/BOM/sim/arch authorities; **not** primary physics/BOM/sim truth | Continuity re-deriving gaps ad hoc; LLM gap inference | ✅ pure projection, no Continuity import (`engineering_readiness.py`) |
