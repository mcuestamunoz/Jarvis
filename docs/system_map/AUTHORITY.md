...
| Which gaps exist + assembly-ready rollup (9 subsystem lines)? | `engineering_readiness.build_engineering_readiness` (ERF-1/ERF-2) — composes closure/BOM/sim/arch/electrical authorities; **not** primary physics/BOM/sim truth. ERF-2 adds `INCOMPATIBLE` verdicts + `electrical_compatibility.py`. **IC 1:** `requirements.defined` via `requirements_declared()` (numeric constraint **or** explicit-none). **Product contract:** `ENGINEERING_READINESS_VISION.md` §11 | Continuity re-deriving gaps ad hoc; LLM gap inference | ✅ pure projection, no Continuity import (`engineering_readiness.py`, `electrical_compatibility.py`) |
...
| Battery / propeller catalog pick? | `battery_catalog_assist.py` / propeller list via orchestrator + `catalog_bind.bind_*_from_catalog` → `component_writers.set_*` | Parallel bind helpers; LLM SKU invention | ✅ library rows only; battery bind does not call `set_motor_component` at orchestrator layer — `set_battery_component` may **conditionally** re-resolve motor OP when `propulsion_resolution` was never voltage-validated or pack voltage is incompatible (v0.3.4 MOP-2); already-validated same-voltage OP is preserved (P2-2/IC2) |
| Motor operating point at bind? | `knowledge/library.py::resolve_operating_point` + `component_writers.set_motor_component` | Ad-hoc thrust/power in writers; exact OP without voltage | ✅ exact match requires explicit `voltage_v` (MOP-1); `propulsion_resolution` JSON carries `voltage_validated` / `resolved_at_voltage_v` (MOP-2) |
...
| Mission mass mirror (`mission_payload_mass_kg`)? | `component_writers.set_mission_component_mass` — sole writer for `cameras`/`radio_module`/`vtx` `mass_g` + its mirror | Direct `current_parameters` write; a second mass source per component | ✅ `COMPONENT_MIRRORED_PARAMS` blocks direct writes (`param_definition_session.py`); fail-closed if identity not already declared |
| Mission power mirror (`mission_accessory_power_w`)? | `component_writers.set_mission_component_power` — sole writer for `cameras`/`radio_module` `power_w` + its mirror. **`vtx` is structurally excluded** (`_MISSION_POWER_KEYS`, narrower than `_MISSION_MASS_KEYS`) — a VTX's cited spec is RF milliwatts, never converted to electrical W | Direct `current_parameters` write; inventing watts from an RF mW citation or a model name | ✅ `COMPONENT_MIRRORED_PARAMS` blocks direct writes; `set_mission_component_power(state, "vtx", ...)` raises `ValueError` by construction |
