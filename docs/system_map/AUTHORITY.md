...
| Battery / propeller catalog pick? | `battery_catalog_assist.py` / propeller list via orchestrator + `catalog_bind.bind_*_from_catalog` → `component_writers.set_*` | Parallel bind helpers; LLM SKU invention | ✅ library rows only; battery bind does not re-call motor writer (IC 2) |
