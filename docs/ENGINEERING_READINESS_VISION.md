...
**Status:** Active (ERF-1 ✅, ERF-2 ✅, Project Closure ✅ §11; Structure + spatial board + Geometry `representar`/glyphs/`mounted_on`/visualizar-3D ✅ — §8; **PRIORIDAD = 3D mapping path** — Scene3D-from-pose CLOSED; next = wheelbase on spec)  
**Type:** Vision / To-be  
**Date:** 2026-09-09 (tag **v0.3.8** · suite **2462**; Scene3D-from-pose CLOSED; Prop/Energy = HD-004 wall; System Optimization **deferred** until pain)
...
> **PRIORIDAD:** [3D mapping path](../.jes/artifacts/engineer_lock_geometry_3d_mapping_path.md) — Scene3D-from-pose [CLOSED](../.jes/artifacts/engineer_smoke_geometry_scene3d_from_pose_b1.md). Next = wheelbase on the bound frame spec (not 4-motor sketch). `"cabe"` last.  
...
- **Execution queue** (what to implement next) lives in [`IMPLEMENTATION_TASKS.md`](./IMPLEMENTATION_TASKS.md) — not in §8 phase history above. **As of 2026-09-09** tag **v0.3.8** · suite **2462**; visualizar-3D CLOSED; Continuity pose writer CLOSED; Scene3D-from-pose CLOSED; **PRIORIDAD = mapping rung 2** (wheelbase on spec). Board B1 layout-on-disk is named debt, not PRIORIDAD. System Optimization **deferred** until pain. Prop/Energy experimental = **HD-004 wall**. MEASURE/CAD is not the default.
