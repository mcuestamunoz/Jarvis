...
**Status:** Active (ERF-1 ✅, ERF-2 ✅, Project Closure ✅ §11; Structure + spatial board + Geometry `representar`/glyphs/`mounted_on`/visualizar-3D ✅ — §8; **PRIORIDAD = 3D mapping path** — Scene3D-from-pose + wheelbase-on-spec first cut CLOSED)  
**Type:** Vision / To-be  
**Date:** 2026-09-09 (tag **v0.3.8** · suite **2466**; wheelbase-on-spec CLOSED; Prop/Energy = HD-004 wall; System Optimization **deferred** until pain)
...
### ✅ Spatial board visor + honest absence B3 — CLOSED (2026-09-05)

> Suite: **2310**. Package still **0.3.8**. Review PASS WITH NOTES.  
> `jarvis board` + `ui/spatial-board/` + `spatial_board.project_spatial_nodes`.  
> Declared-architecture holes → `kind: "slot"` (not BOM). Residual: B1 layout-on-disk — not PRIORIDAD.

### ✅ Geometry `representar` (first families) + sensors claim-copy — CLOSED (2026-09-06→07)

> Suites: Battery **2316** · Motor **2323** · ESC **2327** · FC **2332** · sensors BOM honesty **2336**. Package still **0.3.8**.  
> Declared envelopes as Board text (`length_mm`/`width_mm`/`height_mm`, motor stator/Ø/shaft, FC identity-linked Pixhawk 4).  
> FC path deliberately **not** catalog-shaped (`FLIGHT_CONTROLLER_DIMENSIONS` in `aerial.py`).  
...
> **PRIORIDAD:** Geometry idle. Scene3D-from-pose [CLOSED](../.jes/artifacts/engineer_smoke_geometry_scene3d_from_pose_b1.md). Wheelbase-on-spec [CLOSED](../.jes/artifacts/engineer_smoke_geometry_wheelbase_on_spec_b1.md). `"cabe"` / N-motores = later mapping rungs.  
...
- **Execution queue** (what to implement next) lives in [`IMPLEMENTATION_TASKS.md`](./IMPLEMENTATION_TASKS.md) — not in §8 phase history above. **As of 2026-09-09** tag **v0.3.8** · suite **2466**; visualizar-3D CLOSED; Continuity pose writer CLOSED; Scene3D-from-pose CLOSED; wheelbase-on-spec first cut CLOSED; **PRIORIDAD idle** (4-motor sketch / rungs 3–5 later ★). Board B1 layout-on-disk is named debt, not PRIORIDAD. System Optimization **deferred** until pain. Prop/Energy experimental = **HD-004 wall**. MEASURE/CAD is not the default.
