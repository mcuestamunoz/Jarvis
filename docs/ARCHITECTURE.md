...
- `knowledge/library.py` — capa de biblioteca determinista (no RAG): `ComponentLibrary` carga catálogos JSON de materiales y motores desde `library/`. Lookups exactos (`get_material`, `get_motor`) y sugerencia de motores por **proximidad de KV** (`find_motors_by_kv`, tolerancia ±150). Entradas actuales = productos con `thrust_n` / `weight_g` fijos. Flujo solo-display: el usuario elige `1/2/…` o `no`; nunca auto-apply de parámetros físicos críticos. Si KV conocido y no hay match, se declara el hueco (no se inventa pieza). **Límite / deuda:** el matching no parte de requisitos físicos derivados ni modela “este producto cubre este espacio de diseño” — ver D8 en `IMPLEMENTATION_TASKS.md`.
