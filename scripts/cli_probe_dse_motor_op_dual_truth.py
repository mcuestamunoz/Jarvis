...
Steps:
  1. Bind motor emax_rs2205s_2300 + propeller hq_5045_bn, NO battery yet ->
     no motor_op_power_w=432 lock-in (MOP-1: voltage_v=None never matches
     an exact row).
  2. Bind lipo_6s_10000mah (6S/22.2V) -> MOP-2 conditional revalidation
     confirms honestly at 22.2V; still no stale 432W anywhere.
  3. calcular -> autonomy matches capacity_wh / (motor_count * rating) * 60.
  4. "optimiza para mejorar autonomia" -> explore baseline autonomy equals
     ...
     re-normalized copy).
  5. "aplica la mejor" -> post-apply autonomy equals explore's own promise
     for the applied candidate.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-probe-motor-op-voltage-coherence-") as tmp:
        ...
        motor_power_w = saved3.current_parameters["motor_power_w"]
        motor_count = saved3.current_parameters["motor_count"]
        expected_autonomy = capacity_wh / (motor_count * motor_power_w) * 60
        _assert_close(autonomy3, expected_autonomy, "step3 autonomy_min")
        print(f"✓ Step 3 PASS: autonomy_min={autonomy3} matches {capacity_wh}/({motor_count}x{motor_power_w})x60")

        # ── Step 4: optimiza para mejorar autonomia ─────────────────────
        ...
        assert exploration is not None and exploration.viable, "step4 FAIL: no viable exploration result"
        baseline_autonomy = exploration.baseline_simulation.autonomy_min
        _assert_close(baseline_autonomy, autonomy3, "step4 explore baseline vs calcular")
        print(f"✓ Step 4 PASS: explore baseline autonomy={baseline_autonomy} == calcular autonomy={autonomy3}")

        # ── Step 5: aplica la mejor ──────────────────────────────────────
        promised = exploration.viable[0].simulation.autonomy_min
        result5 = orch.handle_user_text("aplica la mejor", llm)
        ...
        print(f"✓ Step 5 PASS: post-apply autonomy={actual5} == explore promise={promised}")
